    ...
    public GenericBaseRepository() {
        ...
        while (!(genericSuperClass instanceof ParameterizedType)) {
            if(!(genericSuperClass instanceof ParameterizedType)) {
                throw new IllegalStateException("Unable to determine the type arguments because generic superclass neither " +
                        "parameterized type nor class");
            }
            if (genericSuperClass == GenericBaseRepository.class) {
                throw new IllegalStateException("Unable to determine the type arguments because" +
